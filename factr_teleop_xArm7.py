...
class FACTRTeleop(Node, ABC):
    ...
    def __init__(self):
        ...
        #NEW
        #self.sim_joint_pos = np.zeros(7)
        self.sim_joint_pos = np.array(
            self.config["arm_teleop"]["initialization"]["initial_match_joint_pos"][:7]
        )
        ...
        self.simulation = self.declare_parameter(
            "simulation",
            False
        ).value
        ...
        if not self.simulation:
            ...
            self.get_logger().info(
                "Simulation mode enabled: skipping Dynamixel offsets"
            )
        ...
        if not self.simulation:
            ...
            self.get_logger().info(
                "Simulation mode: skipping leader start position matching"
        )
